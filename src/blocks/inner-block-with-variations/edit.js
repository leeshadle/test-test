...
const Edit = ( props ) => {
	...
	const backgroundClasses = classnames(
		`absolute w-full inset-0 z-0 ${ backgroundImage.repeat } ${ backgroundImage.attachment } ${ backgroundImage.customSize ? null : backgroundImage.backgroundSize } ${ backgroundImage.customSize ? null : backgroundImage.position } ${ backgroundImage.opacity }`,
	);
	...
	const containerStyle = {
		'--test-test-box-shadow-color': boxShadow.color,
	}
	...
	if( ! variationName ) {
		return(
			<__experimentalBlockVariationPicker
				...
				instructions={ __( 'Select a variation', 'test-test' ) }
